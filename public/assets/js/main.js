...
(function () {
  ...
  // Initialize menu hide/show toggle
  let menuToggleHidden = document.getElementById('menu-toggle-hidden');
  let menuToggleIcon = document.getElementById('menu-toggle-icon');
  if (menuToggleHidden) {
    menuToggleHidden.addEventListener('click', event => {
      event.preventDefault();
      window.Helpers.toggleMenuHidden();
      
      // Update icon based on menu state
      if (menuToggleIcon) {
        if (window.Helpers.isMenuHidden()) {
          menuToggleIcon.className = 'icon-base ri ri-menu-unfold-line icon-lg';
          menuToggleHidden.setAttribute('title', 'Show Sidebar');
        } else {
          menuToggleIcon.className = 'icon-base ri ri-side-bar-line icon-lg';
          menuToggleHidden.setAttribute('title', 'Hide Sidebar');
        }
      }
    });
  }
